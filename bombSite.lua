			...
			effects = {
				{
					...
					id = 14, -- Fire Damage
					rangeType = 0, -- idk what this does
...
function bombSite.bombDetonate(cellDescription, bombIndex)
	...
	logicHandler.RunConsoleCommandOnObjects(pid, bombExplodeCfg.consoleCommand, cellDescription, {bombIndex}, true)
    logicHandler.DeleteObjectForEveryone(cellDescription, bombIndex)
...
-- Prevent planted bomb from being picked up by any player
function bombSite.OnObjectActivateValidator(eventStatus, pid, cellDescription, objects, targetPlayers)
	for _, object in pairs(objects) do
		if object.refId == bomRefIds.world then
			return customEventHooks.makeEventStatus(false, false)
		end
	end
end
...
customEventHooks.registerHandler("OnObjectActivate",bombSite.OnObjectActivateValidator)
